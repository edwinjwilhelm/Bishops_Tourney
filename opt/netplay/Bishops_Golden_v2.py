...
class GameState:
    ...
    def __init__(self):
        ...
        self.duel_white_origin_ai: bool = False

        self.duel_black_origin_ai: bool = False

        self.duel_epoch: int = 0
...
def _enter_duel_kqbb(bd: Board, state: GameState, white_name: str = "White", black_name: str = "Black"):
    ...
    state.duel_white_origin = PColor.WHITE

    state.duel_black_origin = PColor.BLACK
    ...
    state._pending_duel_move_log = f"---- Duel Start: {white_name} (WHITE) vs {black_name} (BLACK) ----"
    ...
    try:
        eliminate_color(bd, state, PColor.GREY, reason="duel")
        eliminate_color(bd, state, PColor.PINK, reason="duel")
    except Exception:
        pass
    try:
        globals()['forced_turn'] = None
    except Exception:
        pass
    try:
        globals()['turn_i'] = TURN_ORDER.index(PColor.WHITE)
    except Exception:
        pass

    # Preserve AI-control flags from the original finalists so duel inherits

    # the same human/AI assignments without extra toggles.
    ...
    try:
        if hasattr(state, 'player_is_ai') and isinstance(state.player_is_ai, dict):
            ...
            state.player_is_ai[PColor.GREY] = True
            state.player_is_ai[PColor.PINK] = True
            global AI_PLAYERS
            AI_PLAYERS = {c for c, flag in state.player_is_ai.items() if flag}
    except Exception:
        pass
    try:
        state.duel_white_origin_ai = False
        state.duel_black_origin_ai = False
    except Exception:
        pass
    ...
    try:
        ...
        prev_epoch = int(getattr(state, 'duel_epoch', 0) or 0)

        state.duel_epoch = prev_epoch + 1
