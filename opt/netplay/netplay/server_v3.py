...
from .supabase_auth import validate_supabase_token, get_user_email, get_user_id
try:
    import qrcode
...
def _run_profile_query(query: str, params: tuple = ()) -> list[sqlite3.Row]:
...
def get_profile(request: Request):
    ...
    return {"ok": True, "display_name": name, "country": country}

@app.post('/profile')
def set_profile(payload: ProfilePayload, request: Request):
    auth_header = request.headers.get('authorization') or request.headers.get('Authorization')
    uid = get_user_id(auth_header)
    if not uid:
        return JSONResponse({"detail": "Missing token"}, status_code=401)
    name = _sanitize_display_name(payload.display_name)
    if not name:
        raise HTTPException(status_code=400, detail="display_name required")
    country = _sanitize_country_code(payload.country) if payload.country else None
    try:
        _run_profile_query(
        ...
        raise HTTPException(status_code=409, detail="display_name already taken")
    return {"ok": True, "display_name": name, "country": country}
...
class GameHub:
    ...
    async def connect(self, seat: str, user_id: str, ws: WebSocket, country: Optional[str] = None, allow_multi: bool = False) -> tuple[str, str]:
        ...
        async with self.lock:
            ...
            if seat_norm in COLOR_SEATS:
                ...
                if self.room_full() and not existing_seat:
                    raise RoomFullError(self.room_id)
                if existing_seat:
                    self.seat_owners.pop(existing_seat, None)
        await ws.accept()
    ...
    async def handle(self, conn_id: str, msg: Dict[str, Any]):
        ...
        if t == "confirm_new_game":
            ...
            return
        if t == "reset_room":
            # Hard reset of this room only (fresh game state)
            if seat_norm not in COLOR_SEATS:
                await ws.send_text(json.dumps({"type":"error","payload":"Only seated players can reset this room."}))
                return
            await self.reset_game()
            try:
                await self._broadcast({"type": "rooms_update", "rooms": rooms.snapshot()})
...
def _room_number(hub: GameHub) -> int:
    rid = (hub.room_id or '').lower()
    if rid == DEFAULT_ROOM_ID:
        return 1
    m = re.match(rf"^{re.escape(AUTO_ROOM_PREFIX)}-(\d+)$", rid)
    if m:
        try:
...
def _matchmake_assignment(seat_pref: Optional[str]) -> tuple[Optional[GameHub], Optional[str]]:
    preferred = seat_pref if seat_pref in COLOR_SEATS else None
    hubs = sorted(rooms.list_rooms(), key=_room_number)
    for hub in hubs:
        taken = set(hub.seat_owners.keys())
        open_seats = [s for s in COLOR_SEATS if s not in taken]
        if not open_seats:
            continue
        seat = preferred if preferred in open_seats else open_seats[0]
        return hub, seat
    return None, None
...
async def ws_endpoint(ws: WebSocket):
    ...
    auth_token = ws.query_params.get("token")  # Supabase JWT token
    ...
    if auth_token:
        try:
            user_email = get_user_email(auth_token)
            if user_email:
                user_id = user_email  # Use email as user ID for authenticated users
                auth_user = True
        except Exception:
            pass  # Fall back to username if token validation fails
    
    # Fall back to username authentication if no valid token
    if not user_id:
    ...
    seat = seat if seat in COLOR_SEATS else 'AUTO'
    hub, assigned_seat = _matchmake_assignment(seat if seat in COLOR_SEATS else None)
    if not hub or not assigned_seat:
        await ws.accept()
        await ws.send_text(json.dumps({"type": "error", "payload": "No available seats."}))
        await ws.close()
        return
    seat = assigned_seat
    country_code = _sanitize_country_code(country_param)
    allow_multi = auth_user and str(selfplay_param or '').lower() in ('1', 'true', 'yes')
...
@app.get('/matchmake')
def matchmake(seat: str = Query('AUTO')):
    """Return the first room with an open seat, filling Room 1 then Room 2, etc.

    - Room 1 is the default room (DEFAULT_ROOM_ID, typically 'main')
    - Rooms 2..N are the precreated tables (table-2..table-N)
    """
    try:
        seat_pref = (seat or 'AUTO').strip().upper()
        hub, assigned_seat = _matchmake_assignment(seat_pref if seat_pref in COLOR_SEATS else None)
        if hub and assigned_seat:
            return JSONResponse({'ok': True, 'room_id': hub.room_id, 'seat': assigned_seat, 'label': hub.label})
        return JSONResponse({'ok': False, 'error': 'No available seats.'}, status_code=409)
    except Exception as e:
        return JSONResponse({'ok': False, 'error': str(e)}, status_code=500)
