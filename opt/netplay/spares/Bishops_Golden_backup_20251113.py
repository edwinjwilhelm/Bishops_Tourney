...
- All four players play on the full Bishops board. A total of 128 squares are used and in play. The larger 4 corners are safe squares and any King can hide in those squares and be immune from check or checkmate.
...
def gen_moves(board, r, c):
    ...
    if not simulate:
        ...
        # Track safe-corner immunity for kings per rules

        if mover and mover.kind == 'K':

            owner = corner_owner_at(er, ec)

            if gs_obj is not None:

                gs_obj.corner_immune[mover.color] = (owner is not None)
...
def draw_sidebar(screen, moves_list, scroll_offset, show_material, captured_points, auto_elim_enabled, two_stage, grace_active, game_over: bool=False):
    ...
    if game_over:
        ...
        r_lib = pygame.Rect(base_x, y_btn, btn_w, btn_h)

        draw_button(screen, r_lib, "Open Games Library", font_size=16, key='btn_open_library')

        UI_RECTS['btn_open_library'] = r_lib
        ...
        UI_RECTS.pop('btn_open_library', None)



    # ---- Controls grid ----

    # If waiting for Ready, show a prominent Ready button first
    ...
    if waiting:
        ...
        r_ready = pygame.Rect(base_x, y_btn + 34, btn_w, btn_h)

        draw_button(screen, r_ready, "Ready to Play", font_size=18, key='btn_ready')
    ...
    r_hold = pygame.Rect(base_x, y, btn_w, btn_h)
...
class GameState:
    ...
    def __init__(self):
        ...
        # Per-color corner immunity flag (True while king is in corner with both bishops present)
...
def main():
    ...
    gs.freeze_advance = False  # Allow AI to play immediately
            ...
            for ev in evs:
                ...
                if ev.type == pygame.MOUSEBUTTONUP and ev.button == 1:
                    ...
                    try:
                        ...
                        if 'btn_force_duel' in UI_RECTS:
                            ...
                            if UI_RECTS['btn_force_duel'].collidepoint(mx, my):
                                ...
                                try:
                                    ...
                                    # Save completed game if possible (best-effort)
                                    ...
                                    for m in moves_list:
                                        ...
                                        pgn_lines.append(str(m))

                                    meta = {'timestamp': ts, 'moves': len(moves_list)}

                                    alive = board.alive_colors()

                                    if len(alive) == 1:

                                        meta['winner'] = alive[0].name
                                    ...
                                    idx.append({'file': os.path.basename(txt_path), **meta})
                        ...
                        if 'btn_open_library' in UI_RECTS:

                            mx, my = ev.pos

                            if UI_RECTS['btn_open_library'].collidepoint(mx, my):

                                try:

                                    import os, subprocess

                                    folder = os.path.join(os.path.dirname(__file__), 'games')
                            ...
                            if k.startswith('toggle_'):
                                ...
                                try:
                                    ...
                                    if not os.path.exists(tool):
                                        ...
                                        tool = os.path.join(os.path.dirname(SCRIPT_DIR), 'tools', 'export_rules.py')

                                    if os.path.exists(tool):

                                        import subprocess

                                        subprocess.run([sys.executable or 'python', tool, '--out', out_dir, '--quiet'], cwd=os.path.dirname(tool))
                                    ...
                                    if os.name == 'nt':
                                        ...
                                        try:
                                            ...
                                            os.startfile(pdf_path if os.path.exists(pdf_path) else out_dir)
                                    ...
                                    banner = 'Rules exported'

                                except Exception as e:

                                    banner = f"Rules export failed: {e}"
                ...
                try:
                    ...
                    for col in TURN_ORDER:
                        ...
                        if rect is not None and rect.collidepoint(mx, my):
                            ...
                            gs.start_elim_flash(col, ELIM_FLASH_MS)
                            ...
                            if not gs.two_stage_active:
                                ...
                                if thr > 0:
                                    ...
                                    if elim_color and board.find_king(elim_color) is not None:
                                        ...
                                        gs.start_elim_flash(elim_color, ELIM_FLASH_MS)
    ...
    # If a resize is pending and the events have settled, apply it once
        ...
        if gs._pending_cleanup and gs._last_flash_color is not None and gs.elim_flash_color is None:
            ...
            if not getattr(gs, 'chess_lock', False):
        ...
        if not replay_mode and not gs.freeze_advance and not game_over and not getattr(gs, 'two_stage_pause', False):
            ...
            if not legal:
                ...
                if in_check_now:
                    ...
                    gs.start_elim_flash(active_color, ELIM_FLASH_MS)
            ...
            if active_color in AI_PLAYERS:
                ...
                # Advance seat if no forced check priority

                if forced_turn is None:
        ...
        if len(alive_kings) == 1:
            ...
            if not game_over:
                ...
                try:
                    ...
                    pgn_lines = [str(m) for m in moves_list]
                    ...
                    idx.append({'file': os.path.basename(txt_path), 'timestamp': ts, 'winner': winner.name, 'moves': len(moves_list)})
            ...
            if getattr(gs, '_draw_by_repetition', False):
                ...
                if not game_over:
                    ...
                    try:
                        ...
                        idx.append({'file': os.path.basename(txt_path), 'timestamp': ts, 'winner': 'DRAW', 'moves': len(moves_list)})
    ...
    if any(arg.startswith("--self-test") for arg in sys.argv[1:]):
        ...
        try:
            ...
            def _test_buttons_headless():
                ...
                moves_sample = "1. WHITE: K e1->e2"

                plan = [

                    {

                        'name': 'Long board move',

                        'auto_move': True,

                        'capture': lambda env: env['turn_counter'],

                        'check': lambda env, prev: (

                            isinstance(prev, int) and env['turn_counter'] > prev,

                            f"turn counter expected > {prev if isinstance(prev, int) else '?'}; got {env['turn_counter']}"
...

                    {
                        ...
                        'capture': lambda env: env['UI_STATE'].get('mode', 'LONG'),

                        'check': lambda env, prev: (env['UI_STATE'].get('mode', 'LONG') != prev, f"mode stuck at {prev}")

                    },

                    {

                        'name': 'Short board move',
...

                    {
                        ...
                        'capture': lambda env: bool(env['UI_STATE'].get('scaled_mode', False)),

                        'check': lambda env, prev: (bool(env['UI_STATE'].get('scaled_mode', False)) != prev, 'scaled_mode unchanged')

                    },

                    {

                        'name': 'AI strength toggle',

                        'button': 'btn_ai',

                        'capture': lambda env: env['AI_STRENGTH'],

                        'check': lambda env, prev: (env['AI_STRENGTH'] != prev, f"AI_STRENGTH stayed {prev}")

                    },

                    {

                        'name': 'Material toggle off',

                        'button': 'btn_material',

                        'events': ['down', 'up'],

                        'capture': lambda env: env['show_material'],

                        'check': lambda env, prev: (env['show_material'] != prev, 'show_material unchanged on disable')
...

                    {
                        ...
                        'set': {'history': 'snapshot', 'future': 'clear', 'replay_mode': False},

                        'capture': lambda env: (len(env['history']), len(env['future'])),

                        'check': lambda env, prev: (

                            env['replay_mode'] and len(env['history']) == max(prev[0] - 1, 0) and len(env['future']) == prev[1] + 1,

                            f"history/future unexpected: before={prev} after={(len(env['history']), len(env['future']))}"
...

                    {
                        ...
                        'name': 'Resume clears replay',

                        'button': 'btn_resume',

                        'set': {'replay_mode': True},

                        'check': lambda env, prev: (env['replay_mode'] is False, 'replay_mode not cleared')
...

                    {
                        ...
                        'set': {'future': 'snapshot', 'history': 'clear', 'replay_mode': False},

                        'capture': lambda env: (len(env['history']), len(env['future'])),

                        'check': lambda env, prev: (

                            env['replay_mode'] and len(env['history']) == prev[0] + 1 and len(env['future']) == max(prev[1] - 1, 0),

                            f"forward mismatch: before={prev} after={(len(env['history']), len(env['future']))}"
...

                    {
                        ...
                        'name': 'Auto-elim cycle 30',

                        'button': 'btn_autoelim',

                        'capture': lambda env: env['auto_elim_threshold'],

                        'check': lambda env, prev: (

                            env['auto_elim_threshold'] == {0: 18, 18: 30, 30: 0}.get(prev, 18),

                            f"auto_elim_threshold cycle mismatch: prev={prev} now={env['auto_elim_threshold']}"
...

                    {
                        ...
                        'capture': lambda env: env['UI_STATE'].get('view_seat', 'AUTO'),

                        'check': lambda env, prev: (env['UI_STATE'].get('view_seat', 'AUTO') != prev, 'view seat unchanged')
...

                    {
                        ...
                        'capture': lambda env: bool(env['UI_STATE'].get('show_coords', False)),

                        'check': lambda env, prev: (bool(env['UI_STATE'].get('show_coords', False)) != prev, 'coords unchanged')
...

                    {
                        ...
                        'capture': lambda env: env['VERBOSE_DEBUG'],

                        'check': lambda env, prev: (env['VERBOSE_DEBUG'] != prev, 'VERBOSE_DEBUG unchanged')
...

                    {
                        ...
                        'set': {'moves_list': [moves_sample]},

                        'capture': lambda env: export_path.stat().st_mtime if export_path.exists() else None,

                        'check': lambda env, prev: (

                            export_path.exists() and export_path.stat().st_size > 0 and

                            (prev is None or export_path.stat().st_mtime >= prev),

                            'bishops_moves.txt not written'
...

                    {
                        ...
                        'button': 'btn_rules_pdf',

                        'capture': lambda env: rules_txt.stat().st_mtime if rules_txt.exists() else None,

                        'wait_ms': 700,

                        'check': lambda env, prev: (

                            rules_txt.exists() and rules_txt.stat().st_size > 0 and

                            (prev is None or rules_txt.stat().st_mtime >= prev),

                            'rules.txt not updated'
...

                    {
                        ...
                        'button': 'btn_holdstart',

                        'capture': lambda env: env['gs'].hold_at_start,

                        'check': lambda env, prev: (env['gs'].hold_at_start != prev, 'hold_at_start unchanged')

                    },

                    {

                        'name': 'Toggle WHITE AI',

                        'button': 'toggle_white',

                        'capture': lambda env: env['gs'].player_is_ai.get(PColor.WHITE, False),

                        'check': lambda env, prev: (env['gs'].player_is_ai.get(PColor.WHITE, False) != prev,

                                                    'WHITE AI flag unchanged')
...

                    {
                        ...
                        'capture': lambda env: env['gs'].player_is_ai.get(PColor.GREY, False),

                        'check': lambda env, prev: (env['gs'].player_is_ai.get(PColor.GREY, False) != prev,

                                                    'GREY AI flag unchanged')

                    },

                    {

                        'name': 'Toggle BLACK AI',

                        'button': 'toggle_black',

                        'capture': lambda env: env['gs'].player_is_ai.get(PColor.BLACK, False),

                        'check': lambda env, prev: (env['gs'].player_is_ai.get(PColor.BLACK, False) != prev,

                                                    'BLACK AI flag unchanged')
...

                    {
                        ...
                        'capture': lambda env: env['gs'].player_is_ai.get(PColor.PINK, False),

                        'check': lambda env, prev: (env['gs'].player_is_ai.get(PColor.PINK, False) != prev,

                                                    'PINK AI flag unchanged')
...

                    {
                        ...
                        'button': 'btn_force_duel',

                        'wait_ms': 1500,

                        'check': lambda env, prev: True,

                        'set_after': {'gs.player_is_ai': {'WHITE': False, 'GREY': False, 'BLACK': False, 'PINK': False}},

                    },

                    {

                        'name': 'Duel board move',

                        'move': {'from': (CH_MAX-1, CH_MIN+4), 'to': (CH_MAX-2, CH_MIN+4)},
                        ...
                        'wait_for': lambda env: bool(getattr(env['gs'], '_duel_started', False)),
...

                    {
                        ...
                        'check': lambda env, prev: (env['show_material'] != prev, 'show_material unchanged on restore')

                    },

                    {

                        'name': 'New game confirm prompt',

                        'button': 'btn_newgame',

                        'set': {'UI_STATE.confirm_newgame_until': 0},

                        'capture': lambda env: env['UI_STATE'].get('confirm_newgame_until', 0),

                        'check': lambda env, prev: (

                            env['UI_STATE'].get('confirm_newgame_until', 0) > env['ticks'],

                            'confirm window not set'
...

                    {
                        ...
                        'check': lambda env, prev: (
                            ...
                            not env['history'] and not env['future'] and not env['moves_list'] and

                            env['gs'].waiting_ready == env['gs'].hold_at_start,

                            'new game did not reset state'
...

                    {
                        ...
                        'name': 'Ready clears wait',

                        'button': 'btn_ready',

                        'set': {'gs.waiting_ready': True},

                        'check': lambda env, prev: (env['gs'].waiting_ready is False, 'waiting_ready not cleared')

                    },

                    {

                        'name': 'Open games library',

                        'button': 'btn_open_library',

                        'set': {'game_over': True},

                        'set_after': {'game_over': False},
...

                    {
                        ...
                        'name': 'Logs toggle off',

                        'button': 'btn_logs',

                        'capture': lambda env: env['VERBOSE_DEBUG'],

                        'check': lambda env, prev: (env['VERBOSE_DEBUG'] != prev, 'VERBOSE_DEBUG unchanged on disable')
