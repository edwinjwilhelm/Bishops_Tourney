...
SAFE_SQUARES = set().union(*SAFE_CORNERS.values())

EXIT_SQUARES_BY_CORNER = {
    "TL": {(0, CH_MIN), (1, CH_MIN), (CH_MIN, 0), (CH_MIN, 1), (CH_MIN, CH_MIN)},
    "TR": {(0, CH_MAX), (1, CH_MAX), (CH_MIN, CH_MAX), (CH_MIN, CH_MAX + 1), (CH_MIN, CH_MAX + 2)},
    "BL": {(CH_MAX, 0), (CH_MAX, 1), (CH_MAX, CH_MIN), (CH_MAX + 1, CH_MIN), (CH_MAX + 2, CH_MIN)},
    "BR": {(CH_MAX, CH_MAX), (CH_MAX, CH_MAX + 1), (CH_MAX, CH_MAX + 2), (CH_MAX + 1, CH_MAX), (CH_MAX + 2, CH_MAX)},
}

EXIT_SQUARES = set().union(*EXIT_SQUARES_BY_CORNER.values())
    ...
    if kind == 'N':
        ...
        for dr in (-1,0,1):
            ...
            for dc in (-1,0,1):
                ...
                if is_safe_square(nr, nc):
                    if any_other_king_in_safe(board, color):
                        continue
                    if not is_safe_square(r, c):
                        if not has_two_bishops(board, color):
                            continue
