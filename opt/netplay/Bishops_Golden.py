...
- The 2x2 home corner blocks (all 16 corner squares) are out of bounds; no pieces may occupy them.

- A colour is eliminated immediately if its king is checkmated or captured.

- There is no migration to the 8A-8 area.

- King/Queen swap: on its first move only, a colour may swap its king with a queen; the swap consumes that turn.
...
- Corner squares remain blocked in all phases; strict Chess governs the duel.
...
# ----- Corner zones and blocked squares -----

_CORNER_EDGE = BOARD_SIZE - 2
CORNER_RECTS = {
    PColor.GREY:  (0, 0),
    PColor.BLACK: (0, _CORNER_EDGE),
    PColor.WHITE: (_CORNER_EDGE, 0),
    PColor.PINK:  (_CORNER_EDGE, _CORNER_EDGE),
}
...
def is_blocked_square(r: int, c: int) -> bool:
    return (r, c) in BLOCKED_SQUARES


def safe_corner_id(r: int, c: int):
    for key, cells in SAFE_CORNERS.items():
        if (r, c) in cells:
            return key
    return None


def is_safe_square(r: int, c: int) -> bool:
    return safe_corner_id(r, c) is not None


def is_corner_square(r: int, c: int) -> bool:
    return is_blocked_square(r, c) or is_safe_square(r, c)


def any_other_king_in_safe(board: 'Board', color: PColor) -> bool:
    for col in TURN_ORDER:
        if col == color:
            continue
        kp = board.find_king(col)
        if kp and is_safe_square(kp[0], kp[1]):
            return True
    return False


def has_two_bishops(board: 'Board', color: PColor) -> bool:
    try:
        bishops = board.find_bishops(color)
    except Exception:
        bishops = _pieces_of_kind(board, color, 'B')
    return len(bishops) >= 2


def exit_squares_for_corner(corner_key: str):
    return EXIT_SQUARES_BY_CORNER.get(corner_key, set())
...
def _update_corner_state(board: 'Board', gs_obj: 'GameState', color: PColor) -> None:
    ...
    in_corner = bool(kp and is_safe_square(kp[0], kp[1]))
...
def _handle_corner_entry(board: 'Board', gs_obj: 'GameState', color: PColor) -> None:
    ...
    """Apply guardian promotion the first time a king occupies a safe corner."""
    ...
    if any_other_king_in_safe(board, color):

        return

    if not has_two_bishops(board, color):
    ...
    if not gs_obj.corner_promoted[color]:
        ...
        for _, _, piece in bishops:
            piece.kind = 'Q'
        if hasattr(board, '_rebuild_piece_index'):
            board._rebuild_piece_index()
...
class Board:
    ...
    def __init__(self):
        ...
        self.bishop_positions = {PColor.WHITE: set(), PColor.GREY: set(), PColor.BLACK: set(), PColor.PINK: set()}
    ...
    def _setup(self):
        ...
        # initialize cached piece positions
    ...
    def set(self, r, c, p):
        ...
        if self.in_bounds(r,c):
            if p is not None and is_blocked_square(r, c):
                raise ValueError("Illegal placement in blocked corner square")
            ...
            if prev and prev.kind == 'K':
                if self.king_positions.get(prev.color) == (r,c):
                    self.king_positions[prev.color] = None
            if p and p.kind == 'K':
            ...
            # maintain bishop cache
            if prev and prev.kind == 'B':
                self.bishop_positions.get(prev.color, set()).discard((r, c))
            if p and p.kind == 'B':
                self.bishop_positions.setdefault(p.color, set()).add((r, c))
        ...
        for r in range(BOARD_SIZE):
            for c in range(BOARD_SIZE):
                ...
                if p.kind == 'K':
                    ...
                    self.bishop_positions[p.color].add((r, c))

    def find_bishops(self, color: PColor):
        cached = self.bishop_positions.get(color)
        if cached is None:
            self._rebuild_piece_index()
            cached = self.bishop_positions.get(color, set())
        else:
            for r, c in list(cached):
                p = self.get(r, c)
                if not (p and p.color == color and p.kind == 'B'):
                    self._rebuild_piece_index()
                    cached = self.bishop_positions.get(color, set())
                    break
        return [(r, c, self.get(r, c)) for (r, c) in cached]
...
def gen_moves(board, r, c):
    ...
    if kind == 'N':
                ...
                if not p.has_moved:
                    ...
                    if on_home:
                        ...
                        def _castle_dir(drc):
                            ...
                            if is_corner_square(k1r, k1c) or is_corner_square(k2r, k2c):
                                return None
        ...
        try:
            ...
            if SAFE_CORNERS and gs_obj and gs_obj.corner_evict_pending.get(color, False):
                if is_safe_square(r, c):
                    corner_key = safe_corner_id(r, c)
                    exits = exit_squares_for_corner(corner_key)
                    if exits:
                        exit_moves = [(nr, nc) for (nr, nc) in moves if (nr, nc) in exits]
                        if exit_moves:
                            moves = exit_moves
        ...
        if color in (PColor.WHITE, PColor.BLACK):
            ...
            if board.in_bounds(nr,nc) and not is_corner_square(nr, nc) and not board.get(nr,nc):
                ...
                if (not p.has_moved and board.in_bounds(nr2,nc2)
                    ...
                    and not is_corner_square(nr2, nc2) and not board.get(nr2,nc2)):
        ...
        for dr, dc in caps:
            ...
            if board.in_bounds(ar,ac) and not is_corner_square(ar, ac):
    ...
    if not simulate:
        ...
        if gs_obj is not None:
            ...
            if mover_is_king and mover and end_in_safe and not start_in_safe:
        ...
        if col in (PColor.WHITE, PColor.BLACK):
            ...
            for dr,dc in deltas:
                ...
                if board.in_bounds(rr,cc):
                    ...
                    if p and p.color==col and p.kind=='P' and not is_corner_square(r,c) and abs(cc-c)==1:
            ...
            for dr,dc in deltas:
                ...
                if board.in_bounds(rr,cc):
                    ...
                    if p and p.color==col and p.kind=='P' and not is_corner_square(r,c) and abs(rr-r)==1:
...
def king_in_check(board: Board, color: PColor) -> bool:
    ...
    if SAFE_CORNERS and gs_obj and gs_obj.corner_immune.get(color, False):
...
def board_do_move(board: Board, sr, sc, er, ec, simulate=False):
    ...
    if is_corner_square(sr, sc):
        if simulate:
            return None, False, None, {}
        raise ValueError("Illegal move from blocked corner square")

    if is_corner_square(er, ec):
        if simulate:
            return None, False, None, {}
        raise ValueError("Illegal move into blocked corner square")

    start_in_safe = is_safe_square(sr, sc) if mover_is_king else False

    end_in_safe = is_safe_square(er, ec) if mover_is_king else False
    ...
    if mover_is_king and end_in_safe and not start_in_safe:

        if chess_lock_active:

            if simulate:

                return None, False, None, {}

            raise ValueError("Illegal safe-square entry during duel")

        if any_other_king_in_safe(board, mover.color):

            if simulate:

                return None, False, None, {}

            raise ValueError("Illegal safe-square entry: another king already safe")

        if not has_two_bishops(board, mover.color):

            if simulate:

                return None, False, None, {}

            raise ValueError("Illegal safe-square entry: both bishops required")
...
def _remove_color_pieces(board: Board, color: PColor) -> bool:
    ...
    if removed:
        try:
            ...
            if hasattr(board, 'bishop_positions'):
                board.bishop_positions[color] = set()
        except Exception:
            pass
    return removed
